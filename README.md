...
A machine learning pipeline that trains an XGBoost classifier to detect fraudulent credit card transactions, with experiment tracking via Weights & Biases.
...
```bash
wandb login
# Paste your API key from https://wandb.ai/authorize (input is hidden — just paste and hit Enter)
```
...
3. Applies SMOTE to training set only (training set grows to ~454,902 rows after balancing)
